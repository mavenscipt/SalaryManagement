...
namespace SalaryManagement
{
    public partial class frmUpdate : Form
    {
        ...
        private void Aadhar_checkbox_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Aadhar_Button_Click(object sender, EventArgs e)
        {
            ...
            if (!System.IO.Directory.Exists("PDF"))
            {
                ...
                copyFile(destUrl, SourcePath, destUrl);
        ...
        private void Pan_Button_Click(object sender, EventArgs e)
        {
            ...
            if (!System.IO.Directory.Exists("PDF"))
            {
                System.IO.Directory.CreateDirectory(destUrl);
                copyFile(destUrl, SourcePath, destUrl);
                Pan_Path = System.IO.Path.Combine(destUrl, System.IO.Path.GetFileName(SourcePath));
            }
            else if (!System.IO.Directory.Exists(destUrl))
            {
                System.IO.Directory.CreateDirectory(destUrl);
        ...
        private void Election_Button_Click(object sender, EventArgs e)
        {
            ...
            if (!System.IO.Directory.Exists("PDF"))
            {
                System.IO.Directory.CreateDirectory(destUrl);
                copyFile(destUrl, SourcePath, destUrl);
                Election_Path = System.IO.Path.Combine(destUrl, System.IO.Path.GetFileName(SourcePath));
            }
            else if (!System.IO.Directory.Exists(destUrl))
            {
                System.IO.Directory.CreateDirectory(destUrl);
        ...
        void copyFile(string FilePath, string Source, string Destination)
        {
            string destination = System.IO.Path.Combine(FilePath, System.IO.Path.GetFileName(Source));
        ...
        private void Update_Load(object sender, EventArgs e)
        {
           ...
           this.designationTableAdapter.Fill(this.mainDataSet1.Designation);
            ...
            {
                sdr.Read();
                txtEmployeeName.Text = sdr["Name"].ToString();
                PictureBox_Path_textbox.Text = sdr["Photo"].ToString();
                pictureBox1.ImageLocation = sdr["Photo"].ToString();
                ImagePath = sdr["Photo"].ToString();
                cmbSex.SelectedItem = sdr["Sex"].ToString();
                dtBirthDate.Value = DateTime.Parse(sdr["BirthDate"].ToString());
                txtAge.Text = sdr["Age"].ToString();
                string adhar = sdr["AdharNo"].ToString();
                txtAdharFirst.Text = adhar.Substring(0, 4);
                txtAdharSecond.Text = adhar.Substring(4, 4);
                txtAdharThird.Text = adhar.Substring(8, 4);
                txtPermanentAddress.Text = sdr["PermanentAddress"].ToString();
                txtPincode.Text = sdr["Pincode"].ToString();
                txtPersonalMobile1.Text = sdr["PersonalMobile"].ToString();
                txtPersonalMobile2.Text = sdr["PersonalMobile2"].ToString();
                txtFamilyMobile1.Text = sdr["FamilyContact"].ToString();
                ...
                else if (sdr["Residencestatus"].ToString() == "Without Room")
                {
                    rtbWithoutRoom.Checked = true;
                }
                if (sdr["SalaryType"].ToString() == "MonthlyBase")
                {
                    rtbMonthlyBase.Checked = true;
                }
                else if (sdr["SalaryType"].ToString() == "fix Salary")
                {
                    rtbFix.Checked = true;
                }
                else if (sdr["SalaryType"].ToString() == "Daily Wages")
                {
                ...
                txtISFCCode.Text = sdr["ISFCCode"].ToString();
                txtBankName.Text = sdr["BankName"].ToString();
            }
