...
{
    public partial class FrmDailySalaryList : Form
    {
        int SalaryId;
        double UpadEMI;
        double Kharchi;
        Operations op = new Operations();
        public FrmDailySalaryList()
        {
            InitializeComponent();
        }

        private void FrmDailySalaryList_Load(object sender, EventArgs e)
        {
            ComboboxBind();
        }
        ...
        public void Bind()
        {
            ...
            SqlCommand cmd;
            cmd = new SqlCommand("select [Id],[EmployeeName],[BasicSalary],[Kharchi],[Upad],[RoomRent],[Present],[NetSalary],[Date] from tblDailySalary where [EmployeeID] = '" + EmployeeID + "'  Order By  Id DESC");
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Result == DialogResult.Yes)
            {
                ...
                int result = op.DataInsert("Delete from  tblDailySalary where [EmployeeID] = '" + EmployeeID + "' and Id = '" + SalaryId + "' ");
                if (result > 0)
                {
